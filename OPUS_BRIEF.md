...
**§4 D1 Year-in-Pixels poster (2026-07-23)** — `lib/stats.ts` `yearGrid()` + `trackedYears()` (17 fixture tests) and `components/YearPixels.tsx` on the detail page: Mon→Sun SVG year grid, year chips, PNG poster via a hidden 2× canvas. **D1 is COMPLETE**: the **all-trackers composite** shipped the same day — `components/YearComposite.tsx` at the bottom of `/insights` (day-per-pixel strips, one row per tracker, `year-in-pixels-<year>.png`), with the shared colour helpers extracted to pure, tested `lib/pixels.ts`. Both posters are canvas-only: no backend, no migration, nothing to deploy but the build.

**Next → (highest value first)** — **W1 close the reminder loop** (the last P1: the service worker exists but nothing SENDS — §6 W1 has the Vercel-cron + web-push spec; note migrations 12/13 are taken, so reminders start at `14-`, and this introduces the FIRST server-side route + service-key env var in the repo); §6 W4 no-LLM quick-log box; then §9 I4 measure-trend / I5 goal history.
