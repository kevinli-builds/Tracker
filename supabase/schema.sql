...
create table if not exists trackers (
  ...
  -- opt-in to the owner's public share page (see shares below). Default off.
...
-- ---------------------------------------------------------------------------
-- shares: the public share page ("frontpage"). One row per user — deleting it
-- revokes the page, updating token rotates the link. Trackers opt in via
-- trackers.shared. The ONLY anonymous read path is public_share() below; no
-- table has an anon RLS policy. See 13-sharing.sql.
-- ---------------------------------------------------------------------------
...
-- SECURITY DEFINER: reads across RLS, keyed by the unguessable token. Returns
-- only tracker display fields + per-day totals — never notes/resources/step
-- labels. first_day lets the client compute `since` with its local-day rules.
